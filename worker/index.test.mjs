...
test("valid payload builds a safe GitHub issue", () => {
  ...
  assert.match(issue.body, /Submitted through the Fabricon Aerospace feedback form/);
...
test("balance feedback maps to the balance label", () => {
  const issue = buildGitHubIssue({ type: "balance", title: "Lease costs spike", details: "" });

  assert.equal(issue.title, "[Balance Feedback] Lease costs spike");
  assert.deepEqual(issue.labels, ["balance"]);
});
...
test("worker creates a GitHub issue for valid submissions", async () => {
  ...
  globalThis.fetch = async (url, options) => {
    assert.equal(url, "https://api.github.com/repos/Sankofa06/FabriconFeedback/issues");
