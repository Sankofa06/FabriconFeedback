...
export async function handleRequest(request, env = {}) {
  ...
  const repo = env.GITHUB_REPO || "FabriconFeedback";
  ...
  try {
    const response = await fetch(`https://api.github.com/repos/${owner}/${repo}/issues`, {
      ...
      headers: {
        ...
        "User-Agent": "fabricon-feedback-worker",
...
export function validatePayload(payload) {
  ...
  if (!TYPE_CONFIG[type]) {
    return { ok: false, message: "Choose bug, balance, feature, improvement, or other." };
  }
...
function buildIssueBody(details, type, metadata) {
  const lines = [
    "Submitted through the Fabricon Aerospace feedback form.",
    "",
    "## Feedback",
...
async function checkRateLimit(request, env) {
  ...
  const cacheKey = new Request(`https://fabricon-feedback-rate-limit.local/${hash}`);
